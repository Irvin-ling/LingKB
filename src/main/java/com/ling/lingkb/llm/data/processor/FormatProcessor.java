...
public class FormatProcessor extends AbstractTextProcessor {
    /**
     * 0-mark | 1-remove | etc.-doNothing
     */
    @Value("${data.processor.format.code.method}")
    private int dataFormatCodeMethod;
    @Value("${data.processor.format.enable}")
    private boolean dataFormatEnable;
    ...
    String doProcess(String text) {
        ...
        if (dataFormatEnable) {
            ...
            if (dataFormatCodeMethod == 0) {
                text = markCodeBlocks(text, "// CODE BLOCK");
            } else if (dataFormatCodeMethod == 1) {
